...
conversion_rules = [
    ...
    # (I) ~ (XCIX) の変換
    {
        'name': 'カッコの校閲（半角: ローマ数字）',
        'pattern': r'（[ivxlcIVXLC]{1,4}）',
        'replace': lambda match: match.group().replace('（', '(').replace('）', ')'),
        'color': 'cyan'
    },
    # (a) ~ (z) の変換
    {
        'name': 'カッコの校閲（半角: アルファベット）',
        'pattern': r'（[a-zA-Z]）',
        'replace': lambda match: match.group().replace('（', '(').replace('）', ')'),
        'color': 'cyan'
    },
    # (a-1) ~ (z-99) の変換
    {
        'name': 'カッコの校閲（半角: アルファベット-数字）',
        'pattern': r'（[a-zA-Z]-[0-9]{1,2}）',
        'replace': lambda match: match.group().replace('（', '(').replace('）', ')'),
        'color': 'cyan'
    },
    # (a-1-1) ~ (z-99-99) の変換
    {
        'name': 'カッコの校閲（半角: アルファベット-数字-数字）',
        'pattern': r'（[a-zA-Z]-[0-9]{1,2}-[0-9]{1,2}）',
        'replace': lambda match: match.group().replace('（', '(').replace('）', ')'),
        'color': 'cyan'
    },

    # 全角カッコに変換するルール
    # 数字・ローマ数字・アルファベット以外の内容
    {
        'name': 'カッコの校閲（全角: 非数字・非ローマ数字・非アルファベット）',
        'pattern': r'\([^\divxlcIVXLCa-zA-Z\-]+\)',
        'replace': lambda match: match.group().replace('(', '（').replace(')', '）'),
        'color': 'magenta'
    },
    # 漢字・ひらがな・カタカナを含む場合
    {
        'name': 'カッコの校閲（全角: 漢字・ひらがな・カタカナ）',
        'pattern': r'\([ぁ-んァ-ヶ一-龠]+\)',
        'replace': lambda match: match.group().replace('(', '（').replace(')', '）'),
        'color': 'magenta'
    },
