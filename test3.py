...
def process_individual_runs(text_element, original_text, modified_text, rules, log_file):
    ...
    for match in re.finditer('|'.join(rule['pattern'] for rule in rules), original_text):
        ...
        for rule in rules:
            if re.match(rule['pattern'], matched_text):
                ...
                new_elements.append(create_highlighted_run(original_rpr, replaced_text, rule['color']))
                break
...
def process_paragraph(paragraph, log_file, rules):
    ...
    if conversion_info:
        ...
        for text_elem in paragraph.findall('.//w:t', namespaces):
            process_individual_runs(text_elem, full_text, modified_text, rules, log_file)
